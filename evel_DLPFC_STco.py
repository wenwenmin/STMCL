...
datasize = [np.load(f"./data/preprocessed_expression_matrices/DLPFC_data/{name}/preprocessed_matrix.npy").shape[1] for
            name in names]
...
def get_embedding(SAVE_EMBEDDINGS, dataset):
    if SAVE_EMBEDDINGS:
        for fold in range(9, 10):  # 12
            save_embeddings(model_path=f"./model_result_STco/{dataset}/{names[fold]}/best_{fold}.pt",
...
for fold in range(slice_num):
    ...
    gene_list_path = "common_highly_variable_genes.npy"
