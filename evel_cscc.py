...
# datasize = [np.load(f"./data/preprocessed_expression_matrices/cscc_data2/{name}/preprocessed_matrix.npy").shape[1] for
#             name in names]


if SAVE_EMBEDDINGS:
    for fold in range(12):
        save_embeddings(model_path=f"./model_result/cscc_att/{names[fold]}/best_{fold}.pt",
                        save_path=f"./pred_result/cscc_final/embeddings_{fold}/",
                        datasize=datasize)
...
for fold in range(12):
    ...
    method = "weighted"
    # save_path = f"./cscc_pred/{names[fold]}/"
    ...
    value, indices = find_matches(spot_key, image_query, top_k=200)
